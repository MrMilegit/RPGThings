...
public class ItemEmeraldGemRender implements IItemRenderer {
   ...
   public void renderItem(ItemRenderType type, ItemStack item, Object ... data) {
      if(type != ItemRenderType.EQUIPPED && type != ItemRenderType.EQUIPPED_FIRST_PERSON) {
         ...
         if(item.getItemDamage() == 0) {
            Minecraft.getMinecraft().renderEngine.bindTexture(PTTReference.TEXTURE_EMERALD_SMALL);
            PTTReference.MODEL_GHOST_IRON_SMALL.renderAll();
            Minecraft.getMinecraft().renderEngine.bindTexture(PTTReference.TEXTURE_EMERALD_CRYSTAL);
            PTTReference.MODEL_GHOST_IRON_SMALL_CRYSTAL.renderAll();
         } else {
            Minecraft.getMinecraft().renderEngine.bindTexture(PTTReference.TEXTURE_EMERALD_BIG);
